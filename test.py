...
def add_item():
    item = {"item": {"title": "some title", "categoryId": 1}}
...
class FlaskTestCase(unittest.TestCase):
    ...
    def test_post_items(self):
        item = {"item": {"title": 'Some new title here', "categoryId": 2}}
    ...
    def test_put_item(self):
        ...
        r = requests.put(
            self.base_url + '/items/' + str(item4["item"]["id"]),
    ...
    def test_delete_item(self):
        item5 = add_item()
        r = requests.delete(
            self.base_url + '/items/' + str(item5["item"]["id"]))
        response = psql_command(
            'select exists(select 1 from items where id=%s)',
            values=(item5["item"]["id"], ))
        assert str(r.status_code) in "200"
        assert "{}" in r.text
